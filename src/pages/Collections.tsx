...
//   const fabricTypes = [
//     "All",
//     ...Array.from(new Set(products.map((p) => p.fabricType))),
//   ];
...
//       const matchesFabric =
//         selectedFabric === "All" || product.fabricType === selectedFabric;
//       const matchesColors =
...
//         isActive={SALE_ACTIVE}
//         title="🔥 Festive Sale Ends In"
//       />
...
import { ProductCard, APIProduct } from "@/components/ProductCard"; // Import APIProduct type from ProductCard
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import {
  Select,
...
// --- Interfaces ---
// We use APIProduct from ProductCard to ensure consistency
interface Category {
  id: number;
  name: string;
}
...
const Collections = () => {
  ...
  useEffect(() => {
    const fetchMetadata = async () => {
      try {
        // Router URLs from your urls.py:
        // router.register(r'categories', ...) -> /api/categories/
        // router.register(r'fabrics', ...) -> /api/fabrics/
        // router.register(r'colors', ...) -> /api/colors/
        const [catRes, fabRes, colRes] = await Promise.all([
          fetch(`${API_BASE_URL}/categories/`),
          fetch(`${API_BASE_URL}/fabrics/`), // NOTE: Your router says 'fabrics', not 'fabric-types'
          fetch(`${API_BASE_URL}/colors/`),
        ]);

        if (!catRes.ok || !fabRes.ok || !colRes.ok) {
        ...
        setFabricTypes(fabData);
        setAvailableColors(colData);
      } catch (error) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    if (showInStock) params.append("in_stock", "true");
    if (showOnlyDiscount) params.append("discount", "true");
    ...
    if (sortBy !== "default") {
      params.append("sort", sortMapping[sortBy]);
    }

    try {
      // UPDATED ENDPOINT based on your urls.py:
      // path("products/list/", ProductListView.as_view(), name="product-list")
      const response = await fetch(
        `${API_BASE_URL}/products/list/?${params.toString()}`
      );
      ...
      <div>
        ...
        {isFiltersLoading ? (
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              size="sm"
              variant={selectedFabric === "All" ? "default" : "outline"}
            ...
            {fabricTypes.map((fabric) => (
              <Button
                key={fabric.id}
                size="sm"
                variant={selectedFabric === fabric.name ? "default" : "outline"}
  ...
  return (
    <div className="min-h-screen container mx-auto px-3 sm:px-4 py-4 sm:py-8">
      {/* Sale Countdown */}
      {/* URL Updated to match router: /api/flash-sales/ */}
      <SaleCountdown apiUrl={`${API_BASE_URL}/flash-sales/`} />
